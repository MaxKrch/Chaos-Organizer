...
const KEYS_CURRENT_CACHE = [KEY_LAST_CACHE_FILES, KEY_LAST_CACHE_NOTES, KEY_LAST_CACHE_IMAGES];
...
const FILES_FOR_CACHE = ['./', './index.html', './main.js', './main.css', './service-worker.js', './workers/createBlobUrl.web-worker.js', './img/ui/bg-2.jpg', './img/icons/032.png', './img/icons/042.png', './img/icons/052.png', './img/icons/062.png', './img/icons/072.png', './img/icons/082.png', './img/icons/102.png', './img/icons/123.png', './img/icons/133.png', './img/icons/143.png', './img/icons/153.png'];
const HOST_NAMES = {
  // FRONTEND: 'localhost',
...
const requestNewFrontendFile = async event => {
  const response = await fetch(event.request.clone()).catch(err => postMessageToApp({
    type: `fetchError`,
    data: err
  }, event.clientId));
  if (response?.status >= 200 && response?.status < 300) {
    await saveRequestToCache(event.request, response.clone(), KEY_LAST_CACHE_FILES);
  }
  return response;
};
const requestNotesImages = async event => {
  const cachedImage = await getResponseFromCache(event.request.clone(), KEY_LAST_CACHE_IMAGES);
  if (cachedImage) {
    return cachedImage;
  }
  const response = await fetch(event.request.clone()).catch(err => postMessageToApp({
  ...
  if (response?.status >= 200 && response?.status < 300) {
    await saveRequestToCache(event.request, response.clone(), KEY_LAST_CACHE_IMAGES);
  }
  return response;
};
const requestFeedNotes = async event => {
  try {
    ...
    if (response?.status >= 200 && response?.status < 300) {
      await saveRequestNotesToCache(event.request, response.clone(), KEY_LAST_CACHE_NOTES);
      return response;
    }
    newRepeatRequestNotes(event.request.clone(), event.clientId);
    const cachedNotes = await getResponseNotesFromCache(event.request, KEY_LAST_CACHE_NOTES);
    if (cachedNotes) {
      return cachedNotes;
    }
    return response;
...
const removeOldCache = async () => {
  ...
  const cachesForDelete = allCashes.filter(cache => !KEYS_CURRENT_CACHE.includes(cache));
...
const newRepeatRequestNotes = async (request, clientId) => {
  ...
  REPEAT_REQUESTS.NOTES.location.section === `tag` ? REPEAT_REQUESTS.NOTES.location.id = bodyRequest.tag : REPEAT_REQUESTS.NOTES.location.category = bodyRequest.category;
...
const repeatRequestNotes = async request => {
  ...
  const response = await fetch(request.clone()).catch(err => postMessageToApp({
    type: `fetchError`,
    data: err
  }, REPEAT_REQUESTS.NOTES.clientId));
  if (response?.status >= 200 && response?.status < 300) {
    await saveRequestNotesToCache(request, response.clone(), KEY_LAST_CACHE_NOTES);
  ...
  REPEAT_REQUESTS.NOTES.timer = setTimeout(request => {
    repeatRequestNotes(request);
  }, REPEAT_REQUESTS.NOTES.next, request);
};
