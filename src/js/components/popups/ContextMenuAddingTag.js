...
export default class ContextMenuAddingTag extends ContextMenu {
  ...
  #renderTagListElement() {
    ...
    this.tags.exist.forEach((tag) => {
      if (
        !this.tags.note ||
        this.tags.note.findIndex((item) => item.id === tag.id) < 0
      ) {
  ...
  #saveStaticElements() {
    this.staticElements.inputNewTag = this.element.querySelector(
      `[data-id="newTagInput"]`,
    );
    this.staticElements.saveNewTag = this.element.querySelector(
      `[data-id="newTagSave"]`,
    );
  }
  ...
  #createStreams() {
    ...
    const inputNewTag = fromEvent(
      this.staticElements.inputNewTag,
      `input`,
    ).pipe(debounceTime(150));
    this.saveStream(`inputNewTag`, inputNewTag);

    const pressEnterNewTag = fromEvent(
      this.staticElements.inputNewTag,
      `keypress`,
    ).pipe(
      filter((event) => {
  ...
  #subscribeToStreams(subscriberForAction) {
    ...
    this.subscribeToStream(
      `pressEnterNewTag`,
      this.#onPressEnterNewTag.bind(this),
    );
    this.subscribeToStream(
      `clicksStream`,
      this.#onClickByContextMenu.bind(this),
    );
  }
  ...
  #chekAvailableNewTag(event) {
    ...
    const isExistTag = this.tags.exist.findIndex(
      (item) => item.title === newTag,
    );
  ...
  #onPressEnterNewTag() {
    this.staticElements.saveNewTag.dispatchEvent(
