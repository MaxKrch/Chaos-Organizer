...
export default class ModalGeolocation extends Modal {
	...
	#createStreams() {
		const inputCoordsGeolocation = fromEvent(this.staticElements.coords.input, `input`).pipe(
				debounceTime(150),
		)
		...
		const pressEnterGeolocation = fromEvent(this.staticElements.coords.input, `keypress`).pipe(
			filter(event => {
