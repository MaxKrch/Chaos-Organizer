...
import { Subject, fromEvent, throttleTime, debounceTime, filter, map } from 'rxjs';
...
export default class EditingNote extends BaseComponent {
	...
	#updateAttachment(attachment) {
		...
		if(attachment.videos) {
			 attachment.videos.forEach(item => {
		...
		if(attachment.images) {
			attachment.images.forEach(item => {
		...
		if(attachment.audios) {
			attachment.audios.forEach(item => {
		...
		if(attachment.others) {
			attachment.others.forEach(item => {
