...
export default class MediaRecord extends BaseComponent {
  ...
  #renderElement() {
    ...
    if (this.type === `video`) {
      ...
      this.staticElements.video.backContainer.classList.add(
        `record-media__show-back`,
      );

      this.staticElements.video.mainContainer = document.createElement(`div`);
      this.staticElements.video.mainContainer.classList.add(
        `record-media__show-main`,
      );
    ...
    this.staticElements.controll.container.classList.add(
      `record-media__controll`,
    );

    this.staticElements.controll.cancel = document.createElement(`div`);
    this.staticElements.controll.cancel.classList.add(
      `button-icon`,
      `figure-button`,
      `record-media__button`,
      `record-media__cancel`,
    );
    ...
    this.staticElements.controll.save.classList.add(
      `button-icon`,
      `figure-button`,
      `record-media__button`,
      `record-media__save`,
  ...
  #renderVideoBackElement() {
    ...
    videoElement.classList.add(
      `record-media__show-video`,
      `record-media__show-back-video`,
    );
  ...
  #subscribeToStreams(callback) {
    ...
    this.subscribeToStream(
      `clicksOnControllButons`,
      this.#onClickByControllButtons.bind(this),
    );
  }
  ...
  saveRecordedMedia() {
    ...
    const media =
      this.mediaDataChunks?.length > 0
        ? new File(this.mediaDataChunks, fileName, {
            type: `${this.type}/webm`,
          })
        : null;
