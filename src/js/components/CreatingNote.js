...
export default class CreatingNote extends BaseComponent {
  ...
  #createStreams() {
    ...
    const pressEscOnTextField = fromEvent(
      this.staticElements.main.text,
      `keydown`,
    ).pipe(
      filter((event) => {
    ...
    this.saveStream(
      `filesEnterToDropArea`,
      fromEvent(this.staticElements.main.dropArea, `dragenter`),
    );
    this.saveStream(
      `filesLeaveFromDropArea`,
      fromEvent(this.staticElements.main.dropArea, `dragleave`),
    );
  ...
  #subscribeToStreams(callbackForActions) {
    ...
    this.subscribeToStream(
      `dropFilesOnPage`,
      this.#onDropFilesOnPage.bind(this),
    );
    this.subscribeToStream(
      `focusOnTextField`,
      this.#onFocusTextField.bind(this),
    );
    this.subscribeToStream(
      `blurFromTextField`,
      this.#onBlurFromTextField.bind(this),
    );
    this.subscribeToStream(
      `inputTextCreatingNote`,
      this.#onInputTextCreatingNote.bind(this),
    );
    this.subscribeToStream(
      `pressEnterOnTextField`,
      this.#onPressEnterOnTextInput.bind(this),
    );
    this.subscribeToStream(
      `pressEscOnTextField`,
      this.#onPressEscOnTextInput.bind(this),
    );

    this.subscribeToStream(
      `filesEnterToPage`,
      this.#onFilesEnterToPage.bind(this),
    );
    this.subscribeToStream(
      `filesEnterToDropArea`,
      this.#onFilesEnterToDropArea.bind(this),
    );
    this.subscribeToStream(
      `filesLeaveFromDropArea`,
      this.#onFilesLeaveFromDropArea.bind(this),
    );
    this.subscribeToStream(
      `filesLeaveFromApp`,
      this.#onFilesLeaveFromApp.bind(this),
    );
  ...
  #renderMainSection() {
    ...
    this.staticElements.main.container.classList.add(
      `create-note__main-section`,
    );
  ...
  #renderSectionTags() {
    ...
    this.staticElements.tags.container.classList.add(
      `create-note__added-tags`,
      `hidden-item`,
    );

    this.staticElements.tags.list = document.createElement(`ul`);
    this.staticElements.tags.list.classList.add(
      `tags-row`,
      `create-note__added-tags-list`,
    );
  ...
  #renderSectionAttachment() {
    ...
    this.staticElements.attachment.container.classList.add(
      `create-note__attachment`,
      `create-note__attachment_hidden`,
    );

    this.staticElements.attachment.error = document.createElement(`div`);
    this.staticElements.attachment.error.classList.add(
      `create-note__attachment-error`,
      `hidden-item`,
    );

    this.staticElements.attachment.inline = document.createElement(`ul`);
    this.staticElements.attachment.inline.classList.add(
      `create-note__attachment-list`,
      `create-note__attachment-inline`,
      `hidden-item`,
    );

    this.staticElements.attachment.block = document.createElement(`ul`);
    this.staticElements.attachment.block.classList.add(
      `create-note__attachment-list`,
      `create-note__attachment-block`,
      `hidden-item`,
    );
  ...
  #saveElementsMainSection() {
    this.staticElements.main.text =
      this.staticElements.main.container.querySelector(
        `[data-id="newNoteText"]`,
      );
    this.staticElements.main.send =
      this.staticElements.main.container.querySelector(
        `[data-id="newNoteSend"]`,
      );
    this.staticElements.main.geolocation.container =
      this.staticElements.main.container.querySelector(
        `[data-id="newNoteGeolocation"]`,
      );
    this.staticElements.main.geolocation.icon =
      this.staticElements.main.container.querySelector(
        `[data-id="newNoteGeolocationIcon"]`,
      );
    this.staticElements.main.addTag =
      this.staticElements.main.container.querySelector(
        `[data-id="newNoteAddTag"]`,
      );
    this.staticElements.main.recordVoice =
      this.staticElements.main.container.querySelector(
        `[data-id="newNoteRecordVoice"]`,
      );
    this.staticElements.main.recordVoiceDescription =
      this.staticElements.main.container.querySelector(
        `[data-id="newNoteRecordVoiceDescription"]`,
      );
    this.staticElements.main.recordVideo =
      this.staticElements.main.container.querySelector(
        `[data-id="newNoteRecordVideo"]`,
      );
    this.staticElements.main.recordVideoDescription =
      this.staticElements.main.container.querySelector(
        `[data-id="newNoteRecordVideoDescription"]`,
      );
    this.staticElements.main.addFiles =
      this.staticElements.main.container.querySelector(
        `[data-id="newNoteAddFiles"]`,
      );
    this.staticElements.main.addFilesDescription =
      this.staticElements.main.container.querySelector(
        `[data-id="newNoteAddFilesDescription"]`,
      );
    this.staticElements.main.addFilesInput =
      this.staticElements.main.container.querySelector(
        `[data-id="newNoteAddFilesInput"]`,
      );
    this.staticElements.main.dropArea =
      this.staticElements.main.container.querySelector(
        `[data-id="newNoteDropArea"]`,
      );
  }
  ...
  #createAttachmenElement(file) {
    ...
    const style =
      file.type === `image` || file.type === `video` ? `inline` : `block`;
    ...
    this.staticElements.main.geolocation.container.classList.remove(
      `hidden-item`,
    );
    this.staticElements.main.addTag.classList.remove(`hidden-item`);

    this.staticElements.main.recordVoiceDescription.classList.remove(
      `hidden-item`,
    );
    this.staticElements.main.recordVideoDescription.classList.remove(
      `hidden-item`,
    );
    this.staticElements.main.addFilesDescription.classList.remove(
      `hidden-item`,
    );
  ...
  #disableFullPanelCreateNote() {
    ...
    this.staticElements.main.recordVoiceDescription.classList.add(
      `hidden-item`,
    );
    this.staticElements.main.recordVideoDescription.classList.add(
      `hidden-item`,
    );
  ...
  #createModalGeolocation() {
    ...
    this.modal = new ModalGeolocation(
      this.#onClickByModalGeolocation.bind(this),
    );
    ...
    if (this.geolocation.coords) {
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          this.#onPermissionGeolocation.bind(this),
          this.#onErrorGeolocation.bind(this),
        );
        navigator.geolocation.watchPosition(
          this.#onUpdateGeolocationFromNavigator.bind(this),
          this.#onErrorGeolocation.bind(this),
        );
      }
    }
  ...
  #createModalRecord(data) {
    ...
    switch (data.type) {
      case `errorRecord`:
        this.modal = new ModalRecordMedia(
          this.#onClickByModalRecordError.bind(this),
          data,
        );
        break;
      ...
      case `cantSaveMedia`:
        this.modal = new ModalRecordMedia(
          this.#onClickByModalRecordCantSaveMedia.bind(this),
          data,
        );
        break;
    }
  ...
  #removeModal() {
    ...
    this.staticElements.main.geolocation.icon.classList.add(
  ...
  #disableNoteGeolocation() {
    ...
    this.staticElements.main.geolocation.icon.classList.remove(
      `slider-button_active`,
    );
  }
  ...
  #removeTagFromNote(target) {
    ...
    const indexTargetTag = this.note.tags.findIndex(
      (item) => item.id === targetTagId,
    );
  ...
  #addAttachmentError(error) {
    ...
    this.timers.atachmentError = setTimeout(
  ...
  #onPauseVideo(target) {
    const videoContainer = target.closest(`li`);
    const videoPlayButton = videoContainer.querySelector(
      `[data-target-action="playVideoPreview"]`,
    );
  ...
  #removeAttachment(target) {
    ...
    if (
      (textCreatingNote === createNoteStaticElements.placeholder ||
        textCreatingNote.length < 3) &&
      this.note.attachment.image.length === 0 &&
      this.note.attachment.video.length === 0 &&
      this.note.attachment.audio.length === 0 &&
      this.note.attachment.other.length === 0
    ) {
  ...
  #activeDropArea() {
    this.staticElements.main.dropArea.classList.add(
      `create-note__drop-area_active`,
    );
  }

  #deActiveDropArea() {
    this.staticElements.main.dropArea.classList.remove(
      `create-note__drop-area_active`,
    );
  }
  ...
  #showSectionAttachment() {
    ...
    this.staticElements.attachment.container.classList.remove(
      `create-note__attachment_hidden`,
    );
  }
        ...
        switch (isCanFileAddToNote.error) {
          case `bigSize`:
            this.#addAttachmentError(
              `Слишком большой размер файла: ${file.name}`,
            );
            break;
  ...
  #onFocusTextField() {
    ...
    this.staticElements.main.text.classList.remove(
      `create-note__text-body_empty`,
    );
  }
  ...
  #onPressEnterOnTextInput(event) {
    event.preventDefault();
    this.staticElements.main.send.dispatchEvent(
      new MouseEvent(`click`, {
        bubbles: true,
      }),
    );
  ...
  setDataToCreatingNote(data) {
    ...
    if (
      data.text.length > 2 &&
      data.text !== createNoteStaticElements.placeholder
    ) {
      this.#showSendNoteButton();
    }
  ...
  #createMediaRecord(type) {
    ...
    this.mediaRecord = new MediaRecord(
      app,
      type,
      this.#onResponseFromMediaRecord.bind(this),
    );
  }
  ...
  #sendNote() {
    ...
    const text =
      this.staticElements.main.text.innerHTML.trim() ===
      createNoteStaticElements.placeholder
        ? ''
        : this.staticElements.main.text.innerHTML.trim();
