...
export default class FullscreenMedia extends BaseComponent {
	...
	#renderMediaBlock() {
		...
		if(this.source === `fileNote`) {
			...
			this.staticElements.media.slides.classList.add(`fullscreen-media__media_active`);
	...
	#createMediaFile(file) {
		...
		mediaFileElement.append(
			mediaFileMainContainer,
			mediaFileBackContainer,
		)
	...
	async #scrollMedia(direction = `right`) {
		...
		if(direction === `left`) {
			indexNewSlide = this.index.current > 0 ?
				this.index.current - 1 :
				this.index.max 
		}

		if(direction == `right`) {
			indexNewSlide = this.index.current < this.index.max ?
				this.index.current + 1 :
				0
		}
